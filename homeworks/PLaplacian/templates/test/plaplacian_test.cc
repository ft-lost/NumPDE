...
namespace plaplacian::test
{
    ...
    TEST(plaplacian, fixedPointSolvePLaplacian)
    {
        ...
        // Manufactured solution of -div( ||grad u||^(p-2) grad u) = f with vanishing Dirichlet BCs
        ...
        {
            return 2 * M_PI * M_PI * M_PI * (-8 * cos(x[0] * M_PI) * cos(x[0] * M_PI) * cos(x[1] * M_PI) * cos(x[1] * M_PI) + 3 * cos(x[0] * M_PI) * cos(x[0] * M_PI) + 3 * cos(x[1] * M_PI) * cos(x[1] * M_PI)) * sin(x[0] * M_PI) * sin(x[1] * M_PI) / sqrt(-cos(M_PI * (2 * x[0] - 2 * x[1])) - cos(M_PI * (2 * x[0] + 2 * x[1])) + 2);
        };
        ...
        std::cout << std::left << std::setw(10) << "N" << std::left << std::setw(16)
                  << "L2 error" << std::left << std::setw(16) << "L2 order" << std::left << std::setw(16)
                  << "H1 error" << std::left << std::setw(16) << "H1 order" << std::endl;
        std::cout << "---------------------------------------------" << std::endl;
        for (int i = 1; i < reflevels + 1; ++i)
        {
            ...
            std::cout << std::left << std::setw(10) << N << std::left << std::setw(16) << l2err <<std::left << std::setw(16)
                      << std::log(l2errprev / l2err) / std::log(2) <<std::left << std::setw(16) <<  h1serr<<std::left << std::setw(16) << std::log(h1serrprev / h1serr) / std::log(2) << std::endl;
    ...
    TEST(plaplacian, fixedPointSolvePLaplacian2)
    {
        ...
        auto f = [](Eigen::Vector2d x) -> double
        {
            return -M_PI*(0.5*M_PI*M_PI*M_PI*sin(x[0]*M_PI)*sin(x[1]*M_PI)*sin(x[1]*M_PI)*cos(x[0]*M_PI) - 0.5*M_PI*M_PI*M_PI*sin(x[0]*M_PI)*cos(x[0]*M_PI)*cos(x[1]*M_PI)*cos(x[1]*M_PI))*sin(x[1]*M_PI)*cos(x[0]*M_PI)/std::pow(M_PI*M_PI*sin(x[0]*M_PI)*sin(x[0]*M_PI)*cos(x[1]*M_PI)*cos(x[1]*M_PI) + M_PI*M_PI*sin(x[1]*M_PI)*sin(x[1]*M_PI)*cos(x[0]*M_PI)*cos(x[0]*M_PI),1.25) - M_PI*(0.5*M_PI*M_PI*M_PI*sin(x[0]*M_PI)*sin(x[0]*M_PI)*sin(x[1]*M_PI)*cos(x[1]*M_PI) - 0.5*M_PI*M_PI*M_PI*sin(x[1]*M_PI)*cos(x[0]*M_PI)*cos(x[0]*M_PI)*cos(x[1]*M_PI))*sin(x[0]*M_PI)*cos(x[1]*M_PI)/std::pow(M_PI*M_PI*sin(x[0]*M_PI)*sin(x[0]*M_PI)*cos(x[1]*M_PI)*cos(x[1]*M_PI) + M_PI*M_PI*sin(x[1]*M_PI)*sin(x[1]*M_PI)*cos(x[0]*M_PI)*cos(x[0]*M_PI),1.25) + 2*M_PI*M_PI*sin(x[0]*M_PI)*sin(x[1]*M_PI)/std::pow(M_PI*M_PI*sin(x[0]*M_PI)*sin(x[0]*M_PI)*cos(x[1]*M_PI)*cos(x[1]*M_PI) + M_PI*M_PI*sin(x[1]*M_PI)*sin(x[1]*M_PI)*cos(x[0]*M_PI)*cos(x[0]*M_PI),0.25);
        ...
        for (int i = 1; i < reflevels + 1; ++i)
        {
            ...
            std::cout << std::left << std::setw(10) << N << std::left << std::setw(16) << l2err << std::setw(16)
                      << std::log(l2errprev / l2err) / std::log(2) << std::setw(16) <<  h1serr<< std::setw(16) << std::log(h1serrprev / h1serr) / std::log(2) << std::endl;

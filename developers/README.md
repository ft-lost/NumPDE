...
* Plotting is done as follows: The C++ code only stores the data in a `.csv` file (using `std::ofstream`). The plotting is then done by a python script that reads this `.csv` file. The python script is called within the C++ code via the `systemcall::execute(...)` function. See e.g. `BurgersEquation`.

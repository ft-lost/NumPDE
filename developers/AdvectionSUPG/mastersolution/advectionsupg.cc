...
namespace AdvectionSUPG {
  ...
  void enforce_boundary_conditions(
    ...
    for(const auto &edge : fe_space->Mesh()->Entities(1)){
      LF_ASSERT_MSG(edge->RefEl() == lf::base::RefEl::kSegment() , "Entity should be an edge");
      ...
      if((corners(0,0) + corners(0,1))/2. == 1 ||
          (corners(1,0) + corners(1,1))/2. == 0){
    ...
    for(const auto &point : fe_space->Mesh()->Entities(2)){
      LF_ASSERT_MSG(point->RefEl() == lf::base::RefEl::kPoint() , "Entity should be an edge");
    ...
    auto flag_values{
        lf::fe::InitEssentialConditionFromFunction(*fe_space, bd_flags, mf_g_Gamma_in)};
  ...
  Eigen::VectorXd solveRotSUPG(std::shared_ptr<const lf::uscalfe::FeSpaceLagrangeO2<double>> fe_space){
    // Velocity field
  ...
  void cvgL2SUPG(){
    ...
    //lf::mesh::utils::TPTriagMeshBuilder triagMeshBuilder(std::move(mesh_factory_ptr)); //Doesnt quite work
    ...
    const lf::mesh::utils::MeshFunctionGlobal mf_u{[](const Eigen::Vector2d& x)-> double{return std::pow(sin(M_PI * x.norm()),2);}};

    std::cout << "N" << std::setw(20) << "|" << "L2 Error" << std::endl;
    ...
    for(int i=0; i<5; i++) {
      ...
      auto fe_space =
          std::make_shared<lf::uscalfe::FeSpaceLagrangeO2<double>>(multi_mesh.getMesh(i));
      // Compute the solution
      u_h[i] = AdvectionSUPG::solveRotSUPG(fe_space);

      const lf::fe::MeshFunctionFE mf_sol(fe_space , u_h[i]);
      // Compute the error
      err[i] = std::sqrt(lf::fe::IntegrateMeshFunction(*mesh , lf::mesh::utils::squaredNorm(mf_sol - mf_u),  4));
      // Print the error in console
      std::cout << fe_space->LocGlobMap().NumDofs()  << std::setw(20) << "|" << err[i] << std::endl;
      if(i==4){
  ...
  void visSolution(std::shared_ptr<const lf::uscalfe::FeSpaceLagrangeO2<double>> fe_space,Eigen::VectorXd& u){

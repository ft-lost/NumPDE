 ...
 * @return This function returns the augemented finite element Galerkin matrix in sparse format.
 */
/* SAM_LISTING_BEGIN_1 */
Eigen::SparseMatrix<double> augmentMatrix(
  ...
  lf::assemble::COOMatrix<double> A = lf::assemble::COOMatrix<double>(N_dofs+1, N_dofs+1);
#if SOLUTION
...
Eigen::VectorXd solveTestProblem(const lf::assemble::DofHandler &dofh) {
  ...
  lf::uscalfe::ScalarLoadElementVectorProvider elvec_builder(fe_space,
                                                             mf_identity);
  // since we compute the augmented Problem, we add one entry to phi representing eta

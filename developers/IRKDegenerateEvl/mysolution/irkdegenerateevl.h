...
Eigen::VectorXd timesteppingIRKMOLODE(
  ...
  lf::assemble::COOMatrix<double> ISM(N, N);
    ...
    for (int j = 0; j < s; ++j) {
      if (i == j) {
        for (const Eigen::Triplet<double> &M_trp : Mtv) {
          ISM.AddToEntry(M_trp.row() + i * N, M_trp.col() + j * N, M_trp.value());
        }
      }
      for (const Eigen::Triplet<double> &A_trp : Atv) {
        ISM.AddToEntry(A_trp.row() + i * N, A_trp.col() + j * N, tau * Ark(i,j) * A_trp.value());
      }
    }
  }
  ...
  for (int k = 0; k < no_ts; ++k) {
    ...
    // *********** Uncomment the following line and replace REPLACE with your code
     mu += REPLACE * Eigen::Map<Eigen::MatrixXd>(incs.data(), N, s) *  Ark.row(s-1).transpose();
    // clang-format on
    rec(mu);
  }
  return mu;
